...
def main():
    ...
    dist, path, _ = search.dfs(graph, src, dest)
    print(f"DFS: {path} ({dist :.2f})")

    dist, path, _ = search.bfs(graph, src, dest)
    print(f"BFS: {path} ({dist :.2f})")

    dist, path, _ = search.bestfirst(graph, src, dest)
    print(f"Best First: {path} ({dist :.2f})")

    dist, path, _ = search.dijkstra(graph, src, dest)
